...
def get_parser():
    ...
    parser.add_argument('-p', default="pivot", metavar='TARGET', help='target language')
    parser.add_argument('-mt', default="mt", metavar='FP', help='MT dataset source language')
    parser.add_argument('--trainpref', metavar='FP', default="../test_data/summ/train", help='target language')
    parser.add_argument('--validpref', metavar='FP', default='../test_data/summ/valid', help='comma separated, valid language prefixes')
    ...
    parser.add_argument('--mt-dict', default="./data-bin/dict.tgt.txt")
...
def main(args):
    ...
    def make_binary_dataset(input_prefix, output_prefix, lang):
        if lang == args.p:
            dict = dictionary.Dictionary.load(dict_path(args.target_lang))
        else:
    ...
    def make_all(lang, only_train=False):
        if args.trainpref:
            make_dataset(args.trainpref, 'train', lang)

        if args.validpref and not only_train:
            for k, validpref in enumerate(args.validpref.split(',')):
                outprefix = 'valid{}'.format(k) if k > 0 else 'valid'
                make_dataset(validpref, outprefix, lang)
        if args.testpref and not only_train:
    ...
    make_all(args.p, only_train=True)

    if args.mt_dict:
        mt_dict = dictionary.Dictionary.load(args.mt_dict)
        mt_dict.finalize(
            threshold=args.thresholdsrc,
            nwords=args.nwordssrc,
            padding_factor=args.padding_factor,
        )
        mt_dict.save(dict_path(args.mt))
        make_dataset(args.trainpref, 'train', args.mt)
