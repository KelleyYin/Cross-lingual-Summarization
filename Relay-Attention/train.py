...
def train(args, trainer, task, epoch_itr):
    ...
    for i, sample in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        if i < num_batches - 1 and (i + 1) % update_freq > 0:
            ...
            # if epoch_itr.epoch <= 4:
            #     update_attn_teach = True
            # else:
            #     update_attn_teach = False
            update_attn_teach = True
            log_output = trainer.train_step(sample, update_attn=update_attn_teach)
...
def load_checkpoint(args, trainer, epoch_itr, teacher_model=False):
    """Load a checkpoint and replay dataloader to match."""
    checkpoint_path = os.path.join(args.save_dir, args.restore_file)

    if os.path.isfile(checkpoint_path):
        if teacher_model:
            print("Load pre-trained teacher-model from {}".format(checkpoint_path))
            extra_state = trainer.load_teacher_checkpoint(checkpoint_path)
        else:
            print("Load pre-trained student-model from {}".format(checkpoint_path))
...
def load_dataset_splits(task, splits):
    for split in splits:
        if split == 'train':
            task.load_dataset(split, combine=True, only_train=True)
        else:
