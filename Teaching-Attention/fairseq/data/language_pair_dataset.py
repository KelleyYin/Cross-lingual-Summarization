...
def collate(samples, pad_idx, eos_idx, left_pad_source=True, left_pad_target=False):
    ...
    if samples[0]["pivot"] is not None:
        pivot_tokens = merge('pivot', left_pad=False)
        pivot_tokens = pivot_tokens.index_select(0, sort_order)
        
        mt_src_tokens = merge('mt', left_pad=False)
        mt_src_tokens = mt_src_tokens.index_select(0, sort_order)
        
        mt_tgt_tokens = merge('source', 
                              left_pad=left_pad_target,
                              move_eos_to_beginning=True)
        mt_tgt_tokens = mt_tgt_tokens.index_select(0, sort_order)
    else:
        pivot_tokens = None
        mt_src_tokens = None
        mt_tgt_tokens = None
    ...
    return {
        ...
        'net_input': {
            ...
            'mt_src_tokens': mt_src_tokens,
            'mt_tgt_tokens': mt_tgt_tokens
        },
        'target': target,
    }
...
class LanguagePairDataset(FairseqDataset):
    ...
    def __init__(
        ...
        self.pivot_sizes = np.array(pivot_sizes) if pivot_sizes is not None else None
    ...
    def __getitem__(self, index):
        return {
            ...
            'pivot': self.pivot[index] if self.pivot is not None else None,
            'mt': self.mt[index] if self.mt is not None else None,
