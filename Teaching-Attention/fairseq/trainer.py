...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, NHG_teach, MT_teach):
        ...
        self.NHG_teach = NHG_teach.cuda()
        self.MT_teach = MT_teach.cuda()
    ...
    def load_teacher_checkpoint(self, filename):
        utils.load_model_state(filename, self.NHG_teach)
        return None

    def train_step(self, sample, update_params=True, update_attn=True):
        ...
        loss, sample_size, logging_output, oom_fwd = self._forward(sample, update_attn)
        ...
        if update_params:
            ...
            n_correct = sum(log.get("num_correct", 0) for log in logging_outputs)
            ...
            try:
                ...
                self.meters["acc"].acc(n_correct, ntokens)
